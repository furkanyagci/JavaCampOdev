...
public class UserManager implements UserService{
	...
	public void add(User user) {
		...
		if(userInformationNotEmpty(user) && userInformationCharacterLength(user) && gmailFormatControl(user.getEmail()) && gmailVerificationServiceControl(user.getEmail()) && userEmailRegistered(user)) {
			loggers.log("Sisteme kaydýnýz tamalanmak üzere lütfen onay Linkine týklayýnýz.");
			if (loggers.emailUserVerification(user.getEmail())) {
				
				loggers.log("Üyelik iþleminiz baþarýyla tamamlandý.");//Kullanýcýya mesaj gönderiyoruz.
				this.userDao.add(user);
				
			} else {
				
				loggers.log("Üyelik iþlemi baþarýsýz");//Kullanýcýya mesaj gönderiyoruz.
	...
	public void login(String email, String password) {
		...
		if (control) {
			System.out.println("Bigiler doðru sisteme giriþ yapýldý");
		}else {
			System.out.println("Bigiler yanlýþ sisteme giriþ yapýlmadý !");
		}
	...
	boolean userEmailRegistered(User user) {
		...
		if(emailControl) {
			System.out.println("Bu E-mail ile sisteme daha kayýt olunmuþ");
			return false;
		}else {
			return true;
		}
	...
	boolean gmailVerificationServiceControl(String email) {
		...
		if (result) {
			//System.out.println("Gmail doðrulama servisi gmail'i doðruladý");
			return true;
		} else {
			//System.out.println("Gmail doðrulama gmail'i doðrulamadý ! ");
			return false;
		}
	}
